    ...
    def __init__(self, checkpoint_path, tokenizer_path, device="cpu"):
        ...
        if os.path.exists(config_path):
            ...
            self.config = CatConfig(
                ...
                d_model=cfg.get("hidden_size", cfg.get("d_model", 448)),
                n_layers=cfg.get("num_hidden_layers", cfg.get("n_layers", 7)),
                n_heads=cfg.get("num_attention_heads", cfg.get("n_heads", 7)),
                ffn_hidden=cfg.get("intermediate_size", cfg.get("ffn_hidden", 896)),
            ...
            self.config = CatConfig(**{k: v for k, v in ckpt["config"].items() if k in valid_fields})
        else:
            print("Warning: No config found, using defaults")
            self.config = CatConfig()

        self.model = CatLM(self.config).to(self.device)
        ...
        print(f"CatLM loaded: {total/1e6:.1f}M params")
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Chat with Cat")
    p.add_argument("--checkpoint", default="checkpoints_catlm/best_model.pt")
    p.add_argument("--tokenizer", default="data_cat_zh/tokenizer.json")
    p.add_argument("--device", default="cpu")
    p.add_argument("--prompt", "-p", help="Single prompt mode: ask one question and exit")
    args = p.parse_args()

    engine = CatInference(args.checkpoint, args.tokenizer, args.device)
