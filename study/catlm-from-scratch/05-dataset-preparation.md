...
**CatLM 现在的数据生成不再只是“纯随机模板抽样”，而是“场景化生成 + 轻量 prompt 变体扩展 + 碰撞控制”三层一起工作。**

具体来说：

- 高碰撞主题已经大量改成了“场景化 input/output 成对生成”
- 在真正写入样本前，还会对用户输入做一层轻量变体扩展，例如保留原句，或只加上 `小猫，` / `猫猫，` 这类低噪声称呼
...
这里的设计是有意保持克制的。它的目的不是制造大量表面噪声，而是：

- 扩大 `distinct_inputs`
- 让模型见到更多自然口语写法
- 避免验证集只剩很少几个输入字符串

### 4.7.3 全局按 `input` 分组，并按 `category` 目标数切分训练集和验证集
